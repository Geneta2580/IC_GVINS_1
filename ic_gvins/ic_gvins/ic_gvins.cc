...
bool GVINS::gvinsInitializationOptimization() {
    ...
    return summary.termination_type == ceres::CONVERGENCE;  // 若优化收敛则返回1
}
...
void GVINS::gnssOutlierCullingByChi2(ceres::Problem &problem,
    ...
    double chi2_threshold = 7.815;  // 阈值7.815对应的是自由度为3的卡方分布在显著性水平α=0.05时的临界值
    double cost, chi2;
    ...
    for (auto &data : gnsslist_) {
        ...
        if (index >= 0) {
            ...
            if (!isusekernel) { // 当第二次进入优化前，不使用核函数，第一次进入优化前，使用核函数
                ...
                problem.Evaluate(options, &cost, &residuals, nullptr, nullptr);
                gnsslasterr_ = 2*cost;
                gnsslasterr_stamps_ = timelist_.back();
